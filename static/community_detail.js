...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function displayPostDetails(post) {
    ...
    const communityDetailDate = document.getElementById(
      "community_detail_date"
    );
    ...
    communityDetailDate.textContent = new Date(
      post.created_at
    ).toLocaleDateString();
    ...
    comments.forEach((comment) => {
      ...
      commentDiv.innerHTML = `
                <div class="comment_userinfo">
                    ...
                    <span id="comment_nickname">${
                      comment.author.nickname
                    }</span>
                </div>
                <div class="comment_content">
                    <span id="comment_content">${comment.content}</span>
                    <span id="comment_date">${new Date(
                      comment.created_at
                    ).toLocaleTimeString()}</span>
                    <button class="comment_delete_btn" data-id="${
                      comment.id
                    }">삭제</button>
                </div>
            `;
  ...
  function updateCommentDeleteButtons() {
    document.querySelectorAll(".comment_delete_btn").forEach((button) => {
      ...
      const commentAuthorNickname =
        commentDiv.querySelector("#comment_nickname").textContent;
